  ...
  useEffect(() => {
    const fetchAdmins = async () => {
      const { data } = await axios.get(
        "https://suvblog.onrender.com/api/users/admins",
        {
        ...
        {admin && admin.length > 0 ? (
            admin.slice(0, 4).map((element) => {
              return (
                <div key={element._id}>                  
                  <div>
                    <img src={element.photo.url} alt="Avatar" className="md:w-56 md:h-56 object-cover border border-black rounded-full items-center" />
